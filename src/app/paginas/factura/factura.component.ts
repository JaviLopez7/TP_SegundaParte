...
onConfirmarCompra(): void {
  ...
  const cliente_id = 1; // ⚠️ Reemplazá esto con el ID real si lo tenés dinámico

  const productosPayload = this.productos.map(p => ({
    producto_id: p.id,
    cantidad: p.cantidad || 1,
    precio_unitario: p.precio
  }));

  this.facturaService.crearFactura(cliente_id, productosPayload).subscribe({
    next: () => {
      ...
      // Limpieza del carrito si es necesario
      this.productos = [];
    },
    error: (err) => {
