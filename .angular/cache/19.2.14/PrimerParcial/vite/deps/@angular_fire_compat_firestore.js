...
} from "./chunk-Y7RJ6UID.js";
import "./chunk-A5ELU63O.js";
import {
  isPlatformServer
} from "./chunk-CVN2E3UV.js";
import {
  EnvironmentInjector,
  Inject,
  Injectable,
...
function ɵgetDefaultInstanceOf(identifier, provided, defaultApp) {
  if (provided) {
    ...
    const providedUsingDefaultApp = provided.filter((it2) => it2.app === defaultApp);
...
var zoneWrapFn = (it2, taskDone, injector) => {
  return (...args) => {
    if (taskDone) {
      setTimeout(taskDone, 0);
    }
    return runInInjectionContext(injector, () => run(() => it2.apply(void 0, args)));
  };
};
var ɵzoneWrap = (it2, blockUntilFirst, logLevel) => {
  ...
  return function() {
    ...
    try {
      ...
      warnOutsideInjectionContext(it2, logLevel);
      return it2.apply(this, _arguments);
    }
    ...
    const ret = runOutsideAngular(() => it2.apply(this, _arguments));
    if (!blockUntilFirst) {
    ...
    if (ret instanceof Observable) {
      ...
      return run(() => new Promise((resolve, reject) => {
        pendingTasks.run(() => ret).then((it3) => runInInjectionContext(injector, () => run(() => resolve(it3))), (reason) => runInInjectionContext(injector, () => run(() => reject(reason))));
      }));
...
function ɵcacheInstance(cacheKey, moduleName, appName, fn, deps) {
  const [, instance, cachedDeps] = globalThis.ɵAngularfireInstanceCache.find((it2) => it2[0] === cacheKey) || [];
  if (instance) {
...
var ɵauthFactory = (app, zone, useEmulator, tenantId, languageCode, useDeviceLanguage, settings, persistence) => ɵcacheInstance(`${app.name}.auth`, "AngularFireAuth", app.name, () => {
  ...
  if (settings) {
    for (const [k2, v] of Object.entries(settings)) {
      auth.settings[k2] = v;
    }
  }
  if (persistence) {
...
var AngularFireAuth = class _AngularFireAuth {
  ...
  constructor(options, name5, platformId, zone, schedulers, useEmulator, settings, tenantId, languageCode, useDeviceLanguage, persistence, _appCheckInstances) {
    ...
    const auth = of(void 0).pipe(observeOn(schedulers.outsideAngular), switchMap(() => zone.runOutsideAngular(() => import("./index.esm-KLKP7JXW.js"))), map(() => ɵfirebaseAppFactory(options, zone, name5)), map((app) => ɵauthFactory(app, zone, useEmulator, tenantId, languageCode, useDeviceLanguage, settings, persistence)), shareReplay({
      bufferSize: 1,
      refCount: false
    }));
    if (isPlatformServer(platformId)) {
      ...
      const redirectResult = auth.pipe(switchMap((auth2) => auth2.getRedirectResult().then((it2) => it2, () => null)), pendingUntilEvent(this.injector), shareReplay({
        bufferSize: 1,
        refCount: false
      }));
...
var __PRIVATE_FirebaseAppCheckTokenProvider = class {
  ...
  start(e, t) {
    ...
    this.o = (t2) => {
      e.enqueueRetryable(() => onTokenChanged3(t2));
    };
...
var name4 = "@firebase/firestore-compat";
var version4 = "0.3.53";
...
var Firestore2 = class {
  ...
  namedQuery(name5) {
    return namedQuery(this._delegate, name5).then((expQuery) => {
      if (!expQuery) {
        return null;
      }
      return new Query2(
...
var AngularFirestore = class _AngularFirestore {
   ...
   */
  constructor(options, name5, shouldEnablePersistence, settings, platformId, zone, schedulers, persistenceSettings, _useEmulator, auth, useAuthEmulator, authSettings, tenantId, languageCode, useDeviceLanguage, persistence, _appCheckInstances) {
